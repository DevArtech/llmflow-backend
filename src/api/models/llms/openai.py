...
class OpenAILLM(BaseModel):
    ...
    def __init__(self, api_key: str, model_type: str, temperature: float = 0.7, initial_cache: Optional[List[Union[SystemMessage, AIMessage, HumanMessage]]] = None):
        super().__init__(api_key=api_key, model_type=model_type, temperature=temperature)
        self.model = ChatOpenAI(openai_api_key=api_key, model=model_type, temperature=temperature)

        if initial_cache is None:
            self.cache = [SystemMessage(content="""You are an assistant developed by the LLMFlow framework. 
